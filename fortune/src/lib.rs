use clap::{App, Arg};
use rand::{self, rngs::StdRng, seq::SliceRandom, thread_rng, SeedableRng};
...
fn find_files(paths: &[String]) -> MyResult<Vec<PathBuf>> {
    ...
    for path in paths {
        for entry in WalkDir::new(path) {
            let entry = entry.map_err(|e| format!("{}: {}", path, e.into_io_error().unwrap()))?;
...
fn read_fortunes(paths: &[PathBuf]) -> MyResult<Vec<Fortune>> {
    ...
    for path in paths {
        let mut file =
            BufReader::new(File::open(path).map_err(|e| format!("{}: {}", path.display(), e))?);
        loop {
            let count = file.read_until(DELIMITER, &mut buf)?;
            if count == 0 {
                break;
            }
            if count > 2 {
                fortunes.push(Fortune {
                    ...
                    text: String::from_utf8(buf[0..count - 1].to_owned())?
                        .trim()
                        .into(),
                });
            }
...
fn pick_fortune(fortunes: &[Fortune], seed: Option<u64>) -> Option<String> {
    match seed {
        Some(s) => fortunes
            .choose(&mut StdRng::seed_from_u64(s))
